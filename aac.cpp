...
static void print_usage() {
  std::cout << "AAC - Asus Armoury Control CLI\n\n"
            ...
            << "  AAC -G  eco|hybrid|nvidia        Set GPU MUX mode (reboot "
               "required)\n"
            << "  AAC -B  <percent>                Set battery charge limit "
               "(40-100)\n"
...
static void show_status() {
  ...
  std::cout << "Power Source:   "
            << (AsusBattery::is_on_ac() ? "Charger (AC)" : "Battery (DC)")
            << "\n";
  ...
  if (AsusMux::is_supported()) {
    std::cout << "GPU Mode:       "
              << AsusMux::mode_to_string(AsusMux::get_mode()) << "\n";
  }

  // Dynamic boost
  if (AsusGpu::is_dynamic_boost_supported()) {
    std::cout << "Dynamic Boost:  "
              << (AsusGpu::get_dynamic_boost() ? "On" : "Off") << "\n";
  }
...
int main(int argc, char *argv[]) {
  ...
  if (flag == "-P" || flag == "-p") {
    ...
    if (value == "silent")
      mode = AsusMode::Silent;
    else if (value == "balanced")
      mode = AsusMode::Balanced;
    else if (value == "turbo")
      mode = AsusMode::Turbo;
    else {
      std::cerr << "Error: Invalid mode '" << argv[2]
                << "'. Use: silent, balanced, turbo\n";
      return 1;
    }

    bool ac = AsusBattery::is_on_ac();
    if (AsusModes::set_mode_for(mode, ac)) {
      std::cout << "Power mode set to " << argv[2] << " ("
                << (ac ? "AC" : "Battery") << ")\n";
      return 0;
    } else {
  ...
  if (flag == "-G" || flag == "-g") {
    ...
    if (value == "eco" || value == "integrated")
      mode = AsusMux::Mode::Integrated;
    else if (value == "hybrid" || value == "standard")
      mode = AsusMux::Mode::Hybrid;
    else if (value == "nvidia" || value == "ultimate")
      mode = AsusMux::Mode::Nvidia;
    else {
      std::cerr << "Error: Invalid GPU mode '" << argv[2]
                << "'. Use: eco, hybrid, nvidia\n";
      return 1;
    }

    if (AsusMux::set_mode(mode)) {
      std::cout << "GPU mode set to " << AsusMux::mode_to_string(mode)
                << ". Reboot required.\n";
      return 0;
    } else {
  ...
  if (flag == "-DB" || flag == "-db") {
    ...
    if (!AsusGpu::is_dynamic_boost_supported()) {
      std::cerr << "Error: Dynamic boost is not supported (nvidia-powerd not "
                   "found).\n";
      return 1;
    }
